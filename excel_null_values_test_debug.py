    ...
    def test_excel_export_null_values(self):
        ...
        def check_excel_content(response):
            ...
            try:
                ...
                # Look for specific columns that should contain NULL and 0 values
                print("\n=== Checking for NULL and 0 values ===")
                for col in range(1, sheet.max_column + 1):
                    header = headers[col-1] if col-1 < len(headers) else f"Column {col}"
                    value = sheet.cell(row=shooter_row, column=col).value
                    
                    # Check for TF stage with NULL value (.22 caliber)
                    if header and "TF" in str(header) and ".22" in str(header):
                        print(f"Found .22 TF column: {header}, value: {value}")
                        if value == "-":
                            null_value_found = True
                            print(f"✅ Found NULL value displayed as '-' in column {col} ({header})")
                    
                    # Check for TF stage with 0 value (CF caliber)
                    if header and "TF" in str(header) and "CF" in str(header):
                        print(f"Found CF TF column: {header}, value: {value}")
                        if value == "0" or value == 0:
                            zero_value_found = True
                            print(f"✅ Found 0 value displayed as '{value}' in column {col} ({header})")
                ...
                for col in range(1, sheet.max_column + 1):
                    ...
                    if header and "Average" in str(header):
                        value = sheet.cell(row=shooter_row, column=col).value
                        print(f"Found average column: {header}, value: {value}")
                        
                        # Try to convert to float for comparison
                        try:
                            if value is not None:
                                float_value = float(value)
                                print(f"Average as float: {float_value}")
                                ...
                                if 72 <= float_value <= 75:  # Allow some flexibility in how average is calculated
                                    average_found = True
                                    print(f"✅ Found correct average calculation: {value}")
                        except (ValueError, TypeError) as e:
                            print(f"Could not convert average to float: {e}")
                ...
                # Summarize findings
                print("\n=== Test Results ===")
                print(f"NULL values displayed as '-': {'✅ Yes' if null_value_found else '❌ No'}")
                print(f"0 values displayed as '0': {'✅ Yes' if zero_value_found else '❌ No'}")
                print(f"Average calculation correct: {'✅ Yes' if average_found else '❌ No'}")
